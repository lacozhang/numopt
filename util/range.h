...
// Represent a range of form [a, b)
	...
	public:
        ...
        static Range all() {
            return Range(0, std::numeric_limits<T>::max());
        }
...
Range<T> Range<T>::evenDivide(size_t n, size_t i) const {
    auto itv = static_cast<long double>(end_ - begin_) / static_cast<long double>(n);
...
// TODO: copy the code from other's, don't know the original logic here
template<typename T>
