...
public:
  ...
  ~CrfTemplate() {}
  bool ExtractUnigramFeatures(const std::vector<std::vector<std::string>> &raw,
                              std::vector<std::vector<std::string>> &features);
  bool ExtractBigramFeatures(const std::vector<std::vector<std::string>> &raw,
                             std::vector<std::vector<std::string>> &features);
  ...
  public:
    ...
    const std::string &RetrieveFeaturePrefix() { return featprefix_; }

    const std::string &RetrieveFeaturePrefix() const { return featprefix_; }
...
private:
  ...
  void ParseLCCRFTemplateLine(const std::string &featspecs,
                              LccrfTemplateLine &ret);
  void HandleLCCRFTemplateLine(std::string &line, LccrfTemplateLine &feats);

  void ExtractAsTemplateLine(
      const std::vector<std::vector<std::string>> &rawfeatures,
      std::vector<std::string> &extracted, const LccrfTemplateLine &line,
      int currentpos);
  bool SaveTemplateSet(std::ofstream &sink,
                       const std::vector<LccrfTemplateLine> &featspecs);
  bool valid_;
