...
bool LccrFeaturizer::FeaturizeSentence(
    ...
    std::vector<std::vector<int>> &bigramfeats, std::vector<int> &labels) {
  unigramfeats.clear();
  bigramfeats.clear();
  labels.clear();
...
bool LccrFeaturizer::FeaturizeFile(const std::string &featsrc,
                                   const std::string &featbin) {
  namespace fs = boost::filesystem;
  ...
  std::ifstream src(featsrc);
  std::fstream sink(featbin, std::ios_base::binary | std::ios_base::out |
                                 std::ios_base::trunc);
  ...
  while (src.good() || (sentence.size() > 0)) {
    if (sentence.size() > 0) {
      ...
      BOOST_ASSERT_MSG(sentence.size() == unigramfeats.size(),
                       "number not equal");
      BOOST_ASSERT_MSG(sentence.size() == bigramfeats.size(),
                       "number not equal");
...
bool LccrFeaturizer::AccumulateLabelFromLine(
    const std::vector<std::string> &textlabels) {
  for (const std::string &label : textlabels) {
    if (cedar::da<int>::CEDAR_NO_VALUE ==
        label2id_.exactMatchSearch<int>(label.c_str(), label.size())) {
...
bool LccrFeaturizer::FeaturizeWithTrie(
    const trie_t &trie, const std::vector<std::string> &textfeats,
