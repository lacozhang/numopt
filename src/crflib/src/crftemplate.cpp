...
void CrfTemplate::HandleLCCRFTemplateLine(std::string &line,
                                          LccrfTemplateLine &featspec) {
...
void CrfTemplate::ExtractAsTemplateLine(
...
bool CrfTemplate::SaveTemplateSet(
    std::ofstream &sink, const std::vector<LccrfTemplateLine> &featspecs) {
  for (const LccrfTemplateLine &line : featspecs) {
    sink.write(line.RetrieveFeaturePrefix().c_str(),
               line.RetrieveFeaturePrefix().size());
...
void CrfTemplate::ParseLCCRFTemplateLine(const std::string &line,
  ...
  if (comps.size() == 1) {
    featspecs.AddTemplatePart(std::numeric_limits<int>::max(),
...
bool CrfTemplate::IsTriggerConstantFeature(
    const std::tuple<int, int> &featsepc) {
