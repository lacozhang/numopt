...
LccrfModel::LccrfModel() {
  optionsdesc_.add_options()(
      kOrderOption, boost::program_options::value<int>()->default_value(1),
...
void LccrfModel::InitFromData(DataIterator &iterator) {
  ...
  param_->setZero();
  sparseunikeys_.reserve(maxunigramid_ + 1 > 100000 ? 100000
                                                    : (maxunigramid_ + 1));
...
double LccrfModel::Learn(LccrfSamples &samples, LccrfLabels &labels,
                         SparseVector &grad) {
  ...
  for (int i = 0; i < samples.NumSamples(); ++i) {
    ...
    signal::auto_buffer<
        double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount>>
        nodeprobuffer(uniscoresize);
    signal::auto_buffer<
        double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount *
                                        kLabelBufferCount>>
        edgeprobuffer(biscoresize);

    Eigen::MatrixXd nodeprob = Eigen::Map<Eigen::MatrixXd>(
        nodeprobuffer.data(), labelcount, wordcount);
    Eigen::MatrixXd edgeprob = Eigen::Map<Eigen::MatrixXd>(
        edgeprobuffer.data(), labelcount * labelcount, wordcount - 1);

    avgloss += GetNodeAndEdgeProb(unigramfeature, bigramfeature, nodeprob,
                                  edgeprob, label);

    for (int wordpos = 0; wordpos < wordcount; ++wordpos) {
      for (DataSamples::InnerIterator iter(unigramfeature, wordpos); iter;
           ++iter) {
        ...
        for (int unilabel = 0; unilabel <= maxlabelid_; ++unilabel) {
          ...
          sparseparam_->coeffRef(index) +=
              nodeprob(unilabel, wordpos) * iter.value();
        }
      }

      if (wordpos < wordcount - 1) {
        for (DataSamples::InnerIterator iter(bigramfeature, wordpos); iter;
             ++iter) {
          ...
          int index = GetBigramFeatureIndex(iter.col(), label.coeff(wordpos),
                                            label.coeff(wordpos + 1));
          sparseparam_->coeffRef(index) -= iter.value();
          ...
          for (int fromlabel = 0; fromlabel <= maxlabelid_; ++fromlabel) {
            for (int tolabel = 0; tolabel <= maxlabelid_; ++tolabel) {
              ...
              sparseparam_->coeffRef(index) +=
                  edgeprob(edgeindex, wordpos) * iter.value();
            }
          }
        }
      }
  ...
  int nonzeros = sparseunikeys_.size() * (maxlabelid_ + 1) +
                 sparsebikeys_.size() * (maxlabelid_ + 1) * (maxlabelid_ + 1);
  ...
  for (int i = 0; i < sparsekeys_.size(); ++i) {
    ...
    for (int offset = 0; offset <= maxlabelid_;
         ++offset, ++innerindex, ++innervals) {
  ...
  for (int i = 0; i < sparsekeys_.size(); ++i) {
    ...
    for (int offset = 0; offset < (maxlabelid_ + 1) * (maxlabelid_ + 1);
         ++offset, ++innerindex, ++innervals) {
      *innerindex = (index + offset);
      *innervals = sparseparam_->coeff(index + offset);
    }
...
void LccrfModel::Inference(LccrfSamples &samples, LccrfLabels &labels) {
  ...
  for (int i = 0; i < samples.NumSamples(); ++i) {
    ...
    signal::auto_buffer<
        double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount>>
        uniscorebuffer(uniscoresize);
    signal::auto_buffer<
        double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount *
                                        kLabelBufferCount>>
        biscorebuffer(biscoresize);

    signal::auto_buffer<
        double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount>>
        alphabuffer(uniscoresize);
    signal::auto_buffer<
        double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount>>
        betabuffer(uniscoresize);

    Eigen::MatrixXd uniscore = Eigen::Map<Eigen::MatrixXd>(
        uniscorebuffer.data(), labelcount, wordcount);
    Eigen::MatrixXd biscore = Eigen::Map<Eigen::MatrixXd>(
        biscorebuffer.data(), labelcount * labelcount, wordcount - 1);
...
double LccrfModel::Evaluate(LccrfSamples &samples, LccrfLabels &labels,
                            std::string &summary) {
  namespace signal = boost::signals2::detail;
  ...
  for (int i = 0; i < samples.NumSamples(); ++i) {
    ...
    signal::auto_buffer<double, signal::store_n_objects<kLabelBufferCount>>
        partionbuffer(maxlabelid_ + 1);
    ...
    Eigen::MatrixXd alpha =
        Eigen::Map<Eigen::MatrixXd>(alphabuffer.data(), labelcount, wordcount);
    Eigen::MatrixXd beta =
        Eigen::Map<Eigen::MatrixXd>(betabuffer.data(), labelcount, wordcount);

    Eigen::VectorXd lastalpha =
        Eigen::Map<Eigen::VectorXd>(partionbuffer.data(), maxlabelid_ + 1);
    ...
    lastalpha.setZero();
    lastalpha = alpha.col(0) + beta.col(0);
    samplescore = LogLikelihood(LogSumExp(lastalpha), uniscore, biscore, label,
                                wordcount);
    logli += samplescore;
  ...
  strout << "Average log-likelihood " << logli << " over "
         << samples.NumSamples() << " samples" << std::endl;
  ...
  for (int wordpos = 0; wordpos < sample.rows(); wordpos++) {
    for (int labelid = 0; labelid <= maxlabelid_; ++labelid) {
      for (DataSamples::InnerIterator iter(sample, wordpos); iter; ++iter) {
        val(labelid, wordpos) +=
            iter.value() * GetUnigramWeight(iter.col(), labelid);
      }
    }
  }
...
#ifdef _DEBUG
  BOOST_ASSERT_MSG(val.rows() == (maxlabelid_ + 1) * (maxlabelid_ + 1),
                   "Error columns for bigram weights");
  BOOST_ASSERT_MSG(val.cols() == (sample.rows() - 1),
                   "error rows for bigram feature");
#endif // !_DEBUG
  ...
  for (int wordpos = 0; wordpos < sample.rows() - 1; ++wordpos) {
    for (int fromlabel = 0; fromlabel <= maxlabelid_; ++fromlabel) {
      for (int tolabel = 0; tolabel <= maxlabelid_; ++tolabel) {
        ...
        for (DataSamples::InnerIterator iter(sample, wordpos); iter; ++iter) {
          val(index, wordpos) +=
              iter.value() * GetBigramWeight(iter.col(), fromlabel, tolabel);
        }
      }
    }
  }
...
void LccrfModel::ForwardPass(DenseMatrix &node, DenseMatrix &edge,
                             int wordcount, DenseMatrix &alpha) {
  namespace signal = boost::signals2::detail;

  alpha.setZero();
  alpha.col(0) = node.col(0);
  signal::auto_buffer<double, signal::store_n_objects<kLabelBufferCount>>
      transbuffer(maxlabelid_ + 1);
  Eigen::VectorXd trans =
      Eigen::Map<Eigen::VectorXd>(transbuffer.data(), maxlabelid_ + 1);
...
void LccrfModel::BackwardPass(DenseMatrix &node, DenseMatrix &edge,
                              int wordcount, DenseMatrix &beta) {
  namespace signal = boost::signals2::detail;
  signal::auto_buffer<double, signal::store_n_objects<kLabelBufferCount>>
      transbuffer(maxlabelid_ + 1);
  Eigen::VectorXd trans =
...
void LccrfModel::NodeProb(DenseMatrix &alpha, DenseMatrix &beta, int wordcount,
                          DenseMatrix &nodeprob) {
  namespace signal = boost::signals2::detail;
  signal::auto_buffer<double, signal::store_n_objects<kLabelBufferCount>>
      probuffer(maxlabelid_ + 1);
  Eigen::VectorXd prob =
      Eigen::Map<Eigen::VectorXd>(probuffer.data(), maxlabelid_ + 1);
...
void LccrfModel::EdgeProb(DenseMatrix &alpha, DenseMatrix &beta,
                          DenseMatrix &node, DenseMatrix &edge, int wordcount,
                          DenseMatrix &edgeprob) {
  namespace signal = boost::signals2::detail;
  signal::auto_buffer<
      double, signal::store_n_objects<kLabelBufferCount * kLabelBufferCount>>
      edgeprobuffer((maxlabelid_ + 1) * (maxlabelid_ + 1));
  Eigen::VectorXd prob = Eigen::Map<Eigen::VectorXd>(
      edgeprobuffer.data(), (maxlabelid_ + 1) * (maxlabelid_ + 1));
  ...
  for (int wordpos = 0; wordpos < wordcount - 1; ++wordpos) {
    ...
    for (int fromlabel = 0; fromlabel <= maxlabelid_; ++fromlabel) {
      for (int tolabel = 0; tolabel <= maxlabelid_; ++tolabel) {
        int index = fromlabel * (maxlabelid_ + 1) + tolabel;
        prob.coeffRef(index) += alpha(fromlabel, wordpos) +
                                beta(tolabel, nextpos) + edge(index, wordpos) +
                                node(tolabel, nextpos);
      }
    }
...
void LccrfModel::Viterbi1Best(DenseMatrix &node, DenseMatrix &edge,
                              int wordcount, LabelVector &path) {
  namespace signal = boost::signals2::detail;
  signal::auto_buffer<
      int, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount>>
      trackbuffer(wordcount * (maxlabelid_ + 1));
  signal::auto_buffer<
      double, signal::store_n_objects<kTokenBufferCount * kLabelBufferCount>>
      maxpathbuffer(wordcount * (maxlabelid_ + 1));

  Eigen::MatrixXi track = Eigen::Map<Eigen::MatrixXi>(
      trackbuffer.data(), maxlabelid_ + 1, wordcount);
  Eigen::MatrixXd maxpath = Eigen::Map<Eigen::MatrixXd>(
      maxpathbuffer.data(), maxlabelid_ + 1, wordcount);
  ...
  for (int wordpos = 1; wordpos < wordcount; ++wordpos) {
    for (int label = 0; label <= maxlabelid_; ++label) {
      ...
      for (int fromlabel = 0; fromlabel <= maxlabelid_; ++fromlabel) {
        int index = fromlabel * (maxlabelid_ + 1) + label;
        double score = maxpath(fromlabel, wordpos - 1) +
                       edge(index, wordpos - 1) + node(label, wordpos);
        if (score > maxscore) {
          labelid = fromlabel;
          maxscore = score;
        }
...
double LccrfModel::GetNodeAndEdgeProb(DataSamples &unigramfeature,
                                      DataSamples &bigramfeature,
                                      DenseMatrix &nodeprob,
                                      DenseMatrix &edgeprob,
                                      LabelVector &labels) {
  namespace signal = boost::signals2::detail;
  ...
  Eigen::MatrixXd uniscore =
      Eigen::Map<Eigen::MatrixXd>(uniscorebuffer.data(), labelcount, wordcount);
  Eigen::MatrixXd biscore = Eigen::Map<Eigen::MatrixXd>(
      biscorebuffer.data(), labelcount * labelcount, wordcount - 1);
  ...
  lastalpha.setZero();
  lastalpha = alpha.col(0) + beta.col(0);
  return LogLikelihood(LogSumExp(lastalpha), uniscore, biscore, labels,
                       wordcount);
}

double LccrfModel::LogLikelihood(double logpartion, DenseMatrix &node,
                                 DenseMatrix &edge, LabelVector &labels,
                                 int wordcount) {
