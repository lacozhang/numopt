...
int main(int argc, const char *argv[]) {
  ...
  desc.add_options()("help,h", "print out help message")(
      "feat,f", po::value<std::vector<std::string>>(&featsrcs)->required(),
      "source file used to extract feature")(
      "files,i", po::value<std::vector<std::string>>(&inputfiles),
      "files need featurization")(
      "output,o", po::value<std::string>(&outputprefix)->required(),
      "required outout prefix")(
      "template,t", po::value<std::string>(&featemplate)->required(),
      "templates for crf feature")("cutoff,c",
                                   po::value<int>(&cutoff)->default_value(1),
                                   "cutoff values for feature extraction");
  po::variables_map vm;
  try {
