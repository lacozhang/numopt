...
void StochasticDCA<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::InitFromCmd(int argc, const char * argv[]){	
	...
	BOOST_ASSERT_MSG(this->learn_.l1_ >= 0, "l1 regularization must larger or equal to 0");
	BOOST_ASSERT_MSG(this->learn_.l2_ >= 0, "l2 regularization must larget or equal to 0");
	if (this->dualgap_ <= 0) {
		BOOST_LOG_TRIVIAL(warning) << "Duality gap can be negative or 0, set to default value(1e-3)";
		this->dualgap_ = 1e-3;
	}
}

template<class ParameterType, class SampleType, class LabelType, class SparseGradientType, class DenseGradientType>
void StochasticDCA<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::Train()
{
	int itercnt = 0;
	double funcval = 0.0;
	...
	if ((this->learn_.l2_ == 0) &&
		(this->learn_.l1_ != 0)) {
		BOOST_LOG_TRIVIAL(info) << "For Prox-SDCA to work, need to adjust the parameters";
	...
	BOOST_LOG_TRIVIAL(info) << "Optimize with follow regularization: lambda *(alpha/2 * l2_reg + beta * l1_reg)";
	...
	while (itercnt < this->learn_.maxiter_) {
		...
		if (TrainOneEpoch(this->trainiter_->GetAllData(), this->trainiter_->GetAllLabel(), param, dualcoef_, proxcoef_, rndptrs, datanorms, epochsize, loss,
			lambda, alpha, beta)) {
...
template<class ParameterType, class SampleType, class LabelType, class SparseGradientType, class DenseGradientType>
void StochasticDCA<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::UpdateDataNorms(DataSamples& dat, DenseVector& res, int epochsize)
{
	for (int i = 0; i < epochsize; ++i) {
...
template<class ParameterType, class SampleType, class LabelType, class SparseGradientType, class DenseGradientType>
bool StochasticDCA<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::TrainOneEpoch(
	DataSamples& datas, LabelVector& labels,
	DenseVector& param, DenseVector& dualcoef, DenseVector& proxcoef,
	std::vector<int>& rndptrs, DenseVector& datanorms,
	int epochsize, LossFunc loss,
	double lambda, double alpha, double beta)
{
	...
	for (int i = 0; i < epochsize; ++i) {
		...
		switch (loss) {
		case LossFunc::Hinge: {
			update = (1 - pred*label) / (norm*scale) + dualcoef.coeff(index)*label;
...
template class StochasticDCA<DenseVector, DataSamples, LabelVector, SparseVector, DenseVector>;
