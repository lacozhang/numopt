...
void ConjugateGradient<ParameterType, SampleType, LabelType, SparseGradientType,
...
void ConjugateGradient<ParameterType, SampleType, LabelType, SparseGradientType,
  ...
  std::function<double(ParameterType &, DenseGradientType &)> evaluator =
      std::bind(&ConjugateGradient<ParameterType, SampleType, LabelType,
                                   SparseGradientType,
...
ConjugateGradient<ParameterType, SampleType, LabelType, SparseGradientType,
                  DenseGradientType>::Options() {
  boost::program_options::options_description alldesc(
      "Overall options for Conjugate Gradient");
  alldesc.add(this->basedesc_);
  alldesc.add(this->cgdesc_);
...
void ConjugateGradient<ParameterType, SampleType, LabelType, SparseGradientType,
  ...
  this->cgdesc_.add_options()(
      "cg.n",
      boost::program_options::value<int>(&this->restartcounter_)
          ->default_value(20),
      "reset cg status after n iteration")(
      "cg.m",
      boost::program_options::value<std::string>(&this->methodstr_)
          ->default_value("pr"),
      "method for generation: fr/pr")(
      "cg.ls",
      boost::program_options::value<std::string>(&this->lsfuncstr_)
          ->default_value("bt"),
      "line search method: bt/mt");
}
...
template class ConjugateGradient<DenseVector, DataSamples, LabelVector,
                                 SparseVector, DenseVector>;
template class ConjugateGradient<DenseVector, LccrfSamples, LccrfLabels,
                                 SparseVector, DenseVector>;
