...
enum class LineSearchConditionType { None, Armijo, Wolfe, StrongWolfe };

class LineSearcher {
public:
  LineSearcher(const std::string &lsfuncstr, const std::string &lscondstr,
               int maxtries, float alpha = 1e-4, float beta = 0.9,
  ...
  bool
  LineSearch(DenseVector &param, DenseVector &direc, DenseVector &grad,
             double finit, double &stepsize,
             std::function<double(DenseVector &, DenseVector &)> &funcgrad);

  bool BackTrackLineSearch(
      DenseVector &param, DenseVector &direc, DenseVector &grad, double finit,
      double &stepsize,
      std::function<double(DenseVector &, DenseVector &)> &funcgrad);

  bool MoreThuenteLineSearch(
      DenseVector &param, DenseVector &direc, DenseVector &grad, double finit,
      double &stepsize,
      std::function<double(DenseVector &, DenseVector &)> &funcgrad);
...
private:
  ...
  ParseLineSearchConditionString(const std::string &str);
