...
template<class ParameterType,
	...
	class StochasticVRG : public OptMethodBase<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType> {
	public:
		...
		StochasticVRG(typename OptMethodBaseType::ModelSpecType& model)
			: OptMethodBaseType(model), svrgdesc_("Options for Stochastic Variance Reduction Gradient") {
			InitCmdDescription();
			ResetState();
		}
	...
	private:
		void InitCmdDescription() {
			this->svrgdesc_.add_options()
				(this->kSVRGLearningRateOption, boost::program_options::value<double>(&this->learn_.learningrate_)->default_value(1e-5), "learning rate for svrg")
				(this->kSVRGFrequencyOption, boost::program_options::value<int>(&this->svrginterval_)->default_value(2), "Update optimial estimated parameter");
		}
		void ResetState() {
		}
		int svrginterval_;
...
const char* StochasticVRG<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::kSVRGLearningRateOption = "svrg.lr";

template<class ParameterType, class SampleType, class LabelType, class SparseGradientType, class DenseGradientType>
const char* StochasticVRG<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::kSVRGFrequencyOption = "svrg.iter";
#endif // !__SVRG_H__
