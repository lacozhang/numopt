...
namespace mltools {
  App *App::Create(const std::string &confStr) {
    ...
    Config conf;
    CHECK(google::protobuf::TextFormat::ParseFromString(confStr, &conf)) << "failed to parse conf: " << confStr;
    auto role = MyRole();
    App *app = nullptr;
    if(conf.has_darlin()) {
      switch (role) {
    ...
    CHECK(app) << "fail to create " << conf.ShortDebugString() << "at " << MyNode().ShortDebugString();
    return app;
  }
}
