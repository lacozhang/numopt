...
DataIteratorBase<SampleType, LabelType>::DataIteratorBase()
    : batchdesc_("Data iteration options") {
  batchsize_ = 0;
  dataindex_ = 0;
  realbatchsize_ = 0;
...
template <class SampleType, class LabelType>
void DataIteratorBase<SampleType, LabelType>::InitFromCmd(int argc,
                                                          const char *argv[]) {
...
template <class SampleType, class LabelType>
bool DataIteratorBase<SampleType, LabelType>::GetNextBatch(
    SampleType &databatch, LabelType &labelbatch) {
  ...
  for (size_t startindex = dataindex_; dataindex_ < endindex; ++dataindex_) {
    data_->FeatureCopyAtIndex(databatch, dataindex_ - startindex,
                              datapointers_[dataindex_]);
    data_->LabelCopyAtIndex(labelbatch, dataindex_ - startindex,
                            datapointers_[dataindex_]);
  }
  return true;
}
...
void DataIteratorBase<SampleType, LabelType>::ConstructCmdOptions() {
  batchdesc_.add_options()(
      kBaseBatchSizeOption,
      boost::program_options::value<int>()->default_value(1),
      "batch size for iteration, will be ignored for some optimization "
      "methods")(kBaseRandomSeedOption,
                 boost::program_options::value<int>()->default_value(0),
                 "random seed for date pertubation");
}
...
template class DataIteratorBase<NNModel::NNSequenceFeature,
