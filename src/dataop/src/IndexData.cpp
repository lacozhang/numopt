...
template<>
void IndexData<NNModel::NNQueryFeature, NNModel::NNQueryLabel>::ResizeFeature(NNModel::NNQueryFeature& feature, size_t numsample){
	feature.Features().resize(numsample);
}
template<>
void IndexData<NNModel::NNQueryFeature, NNModel::NNQueryLabel>::ResizeLabel(NNModel::NNQueryLabel& label, size_t numsample){
...
size_t IndexData<NNModel::NNQueryFeature, NNModel::NNQueryLabel>::SampleSize(){
	if (features_->NumSamples() != labels_->NumSamples()){
		BOOST_LOG_TRIVIAL(error) << "Samples of feature do not equal samples of labels";
	}
	return features_->NumSamples();
}
template<>
