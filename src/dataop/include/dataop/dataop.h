...
enum TrainDataType { kLibSVM, kLCCRF, kSemiCRF, kNNQuery, kNNSequence };

template <TrainDataType T, class Feat, class Label> class DataLoader {
...
public:
  ...
  size_t MaxFeatureId() { return maxfeatid_; }

  void SetCutoff(size_t v) { cutoff_ = v; }

  void SetModelMetaInfo(
      const boost::shared_ptr<DataLoader<T, Feat, Label>> &infosrc);

  size_t GetMaxFeatureId() const { return maxfeatid_; }

  // for lccrf
  int GetMaxUnigramFeatureId() const { return maxunifeatid_; }

  int GetMaxBigramFeatureId() const { return maxbifeatid_; }

  int GetMaxLabelId() const { return maxlabelid_; }

  // for all
  boost::shared_ptr<Feat> &GetData() { return features_; }

  boost::shared_ptr<Label> &GetLabels() { return labels_; }

  bool IsValidDatset() const { return valid_; }
