...
public:
  ...
  da()
      : tracking_node(), _array(0), _ninfo(0), _block(0), _bheadF(0),
        _bheadC(0), _bheadO(0), _capacity(0), _size(0), _no_delete(false),
        _reject() {
    STATIC_ASSERT(
  ...
  size_t commonPrefixSearch(const char *key, T *result, size_t result_len,
                            size_t len, size_t from = 0) const {
    size_t num = 0;
  ...
  value_type traverse(const char *key, size_t &from, size_t &pos,
                      size_t len) const {
    union {
      int i;
      value_type x;
...
#ifdef USE_REDUCED_TRIE
    const int to =
        _array[from].value >= 0 ? static_cast<int>(from) : _follow(from, 0, cf);
  ...
  void erase(size_t from) {
    // _test ();
#ifdef USE_REDUCED_TRIE
    int e = _array[from].value >= 0 ? static_cast<int>(from)
                                    : _array[from].base() ^ 0;
  ...
  int open(const char *fn, const char *mode = "rb", const size_t offset = 0,
           size_t size_ = 0) {
    FILE *fp = std::fopen(fn, mode);
    if (!fp)
      return -1;
  ...
  int next(size_t &from, size_t &len, const size_t root = 0) {
    ...
    return c ? begin(from = static_cast<size_t>(_array[from].base()) ^ c, ++len)
             : CEDAR_NO_PATH;
  }
...
private:
  ...
  // follow/create edge
  template <typename T> int _follow(size_t &from, const uchar &label, T &cf) {
    int to = 0;
    const int base = _array[from].base();
  ...
  int _find(const char *key, size_t &from, size_t &pos,
    ...
    if (_array[from].value >= 0) // get value from leaf
      return pos == len ? _array[from].value
                        : CEDAR_NO_PATH; // only allow integer key
#endif
    ...
    for (int to = 0; to < _size; ++to) {
      ...
      if (const uchar label = static_cast<uchar>(base ^ to)) // skip leaf
        _push_sibling(static_cast<size_t>(from), base, label,
                      !from || _ninfo[from].child ||
                          _array[base ^ 0].check == from);
    }
  }
  void _restore_block() {
  ...
  void _push_sibling(const size_t from, const int base, const uchar label,
