...
namespace mltools {
  ...
  public:
    ...
    template <typename C>
    void countUniqIndex(const DArray<I> &idx, DArray<I> *uniqIdx, DArray<C> *idxFreq);
    
    template <typename C>
    void countUniqIndex(const MatrixPtr<V> &mat, DArray<I> *uniqIdx, DArray<C> *idxFreq) {
...
#pragma pack(pop)
  ...
  template <typename I, typename V>
  template <typename C>
  void Localizer<I,V>::countUniqIndex(const DArray<I> &idx, DArray<I> *uniqIdx, DArray<C> *idxFreq) {
    if(idx.empty()) {
      return;
    }
  ...
  MatrixPtr<V> Localizer<I,V>::remapIndex(const DArray<I> &idxdict) {
    CHECK(mat_);
    return remapIndex(mat_->info(), mat_->offset(), mat_->index(), mat_->value(), idxdict);
  }
  ...
  MatrixPtr<V> Localizer<I,V>::remapIndex(const MatrixInfo &info,
    ...
    return MatrixPtr<V>(new SparseMatrix<I,V>(newInfo, newOffset, newIndex, newValue));
  }
}
