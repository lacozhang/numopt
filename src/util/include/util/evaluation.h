...
template <typename V> class Evaluation {
public:
  static V auc(const DArray<V> &label, const DArray<V> &predict);

  static V accuracy(const DArray<V> &label, const DArray<V> &predict,
                    V threshold = 0);

  static V logloss(const DArray<V> &label, const DArray<V> &predict);
};
...
V Evaluation<V>::auc(const DArray<V> &label, const DArray<V> &predict) {
  ...
  std::sort(buff.data(), buff.data() + n,
            [](const Entry &a, const Entry &b) { return a.pred_ < b.pred_; });
  V area = 0, cumTp = 0;
  for (int i = 0; i < n; ++i) {
    if (buff[i].label_ > 0) {
      cumTp += 1;
...
template <typename V>
V Evaluation<V>::accuracy(const DArray<V> &label, const DArray<V> &predict,
                          V threshold) {
  int n = label.size();
  CHECK_EQ(n, predict.size());
  double tp = 0;
