...
public:
  ...
  da()
      : tracking_node(), _array(0), _tail(0), _tail0(0), _ninfo(0), _block(0),
        _bheadF(0), _bheadC(0), _bheadO(0), _capacity(0), _size(0), _quota(0),
        _quota0(0), _no_delete(false), _reject() {
    STATIC_ASSERT(
  ...
  void suffix(char *key, size_t len, npos_t to) const {
    ...
    if (const int offset = static_cast<int>(to >> 32)) {
      ...
      if (len > len_tail)
        ...
        len_tail = len, len = 0;
      std::memcpy(&key[len], &_tail[static_cast<size_t>(offset) - len_tail],
                  len_tail);
    }
    while (len--) {
...
#ifdef USE_EXACT_FIT
      _quota += needed > *_length || needed > MAX_ALLOC_SIZE
                    ? needed
  ...
  void set_array(void *p, size_t size_ = 0) { // ad-hoc
    ...
    _size =
        static_cast<int>(size_ / unit_size() + (size_ % unit_size() ? 1 : 0));
    _no_delete = true;
  }
...
private:
  ...
  // follow/create edge
  template <typename T> int _follow(npos_t &from, const uchar &label, T &cf) {
    int to = 0;
