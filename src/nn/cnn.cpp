...
namespace NNModel {
    ...
    void CNNModel::Init() {
        ...
        embedlayer_ = boost::make_shared<EmbeddingLayer>(parambae, gradbase, vocabsize_, embedsize_);
        parambae += vocabsize_*embedsize_;
        gradbase += vocabsize_*embedsize_;
        convlayer_ = boost::make_shared<TextConvLayer>(parambae, gradbase, convfilters_, convsize_, embedsize_, 1);
        poolinglayer_ = boost::make_shared<TextMaxPoolingLayer>(poolstack_, embedsize_);
        ...
        hiddenlayer_ = boost::make_shared<LinearLayer>(parambae, gradbase, poolstack_*embedsize_, hiddensize_);
        parambae += poolstack_*embedsize_*hiddensize_;
        gradbase += poolstack_*embedsize_*hiddensize_;
        bowhiddenlayer_ = boost::make_shared<EmbeddingSumLayer>(parambae, gradbase, vocabsize_, hiddensize_);
        ...
        outputlayer_ = boost::make_shared<LinearLayer>(parambae, gradbase, hiddensize_, labelsize_);
        parambae += hiddensize_ * labelsize_;
        gradbase += hiddensize_ * labelsize_;

        bowoutputlayer_ = boost::make_shared<EmbeddingSumLayer>(parambae, gradbase, vocabsize_, labelsize_);
