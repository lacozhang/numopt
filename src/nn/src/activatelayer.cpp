...
void ActivateLayer::Forward(const RealVector &input,
...
void ActivateLayer::InputGrad(const RealVector &input,
                              const boost::shared_ptr<RealVector> &gradin,
                              boost::shared_ptr<RealVector> &gradout) {
  inputgrad_->resizeLike(input);
  inputgrad_->setZero();
  for (int i = 0; i < input.size(); ++i) {
    inputgrad_->coeffRef(i) =
        func_->derivate(input.coeff(i), output_->coeff(i), gradin->coeff(i));
  }
  gradout = inputgrad_;
}
} // namespace NNModel
