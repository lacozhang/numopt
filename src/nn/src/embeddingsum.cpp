...
EmbeddingSumLayer::EmbeddingSumLayer(double *param, double *grad, int vocabsize,
                                     int embeddingsize)
    : param_(param, vocabsize, embeddingsize),
      grad_(grad, vocabsize, embeddingsize) {
  vocabsize_ = vocabsize;
  embedsize_ = embeddingsize;
  output_ = boost::make_shared<RealVector>();
  output_->resize(embedsize_);
}
void EmbeddingSumLayer::Forward(const DataSamples &input,
...
void EmbeddingSumLayer::ParamGrad(const DataSamples &input,
