...
namespace NNModel {
    ...
    class CrossEntropyLoss : public LossLayerBase<RealVector, double, int, RealVector> {
    public:
        ...
        double FAST_EXP(double val)
        {
            ...
            *(int*)((unsigned char *)&r + 4) = static_cast<int>(EXP_A * (val)+(1072693248 - EXP_C));
            return r;
        }


        double Forward(const RealVector& input, const int& golden);
        void BackWard(const RealVector& input, const int& golden, boost::shared_ptr<RealVector>& grads);
