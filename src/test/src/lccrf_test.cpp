...
BOOST_AUTO_TEST_CASE(LccrfModelLoad) {
  ...
  const char *argv[] = {"test", "--data.train", "lccrf_test.bin", "--batch",
                        "1"};
  int argc = 5;
  ...
  iterator.SetDataSet(
      boost::make_shared<IndexData<LccrfSamples, LccrfLabels>>(samples, label));
  ...
  while (iterator.GetNextBatch(batchsample, batchlabel)) {
    ...
    BOOST_ASSERT_MSG((densegrad - sparsegrad).norm() < 1e-3,
                     "Error, difference between dense & sparse gradient");
