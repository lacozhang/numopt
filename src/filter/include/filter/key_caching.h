...
namespace mltools {
  ...
  public:
    void encode(Message *msg) {
      ...
      auto cacheK = std::make_pair(msg->task_.key_channel(), Range<Key>(msg->task_.key_range()));
      Lock lk(mu_);
  ...
  private:
    ...
    bool isDone(const Task &task) {
      return (!task.request() ||
              (task.has_param() && task.param().push()));
    }
    
    std::unordered_map<std::pair<int, Range<Key>>, std::pair<uint32_t, DArray<char>>> cache_;
    
    // calculate the signature using the first maxSigLen_*4 bytes to accelerate the computation.
