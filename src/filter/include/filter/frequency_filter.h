...
template <typename K, typename V> class FrequencyFilter {
public:
...
DArray<K> FrequencyFilter<K, V>::queryKeys(const DArray<K> &key,
                                           int threshold) {
...
template <typename K, typename V>
void FrequencyFilter<K, V>::insertKeys(const DArray<K> &keys,
                                       const DArray<V> &counts) {
