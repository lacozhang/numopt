...
namespace mltools {
  ...
  private:
    ...
    template <typename V>
    DArray<char> convert(const DArray<char> &array, bool encode, int nbytes, FilterConfig::FixedFloatConfig *conf) {
      CHECK_GT(nbytes, 0);
      CHECK_LT(nbytes, 8);
    ...
    void convert(Message *msg, bool encode) {
      ...
      for(int i=0; i<n; ++i) {
        ...
        if(type == DataType::FLOAT) {
          msg->value_[i] = convert<float>(msg->value_[i], encode, filterConf->num_bytes(), filterConf->mutable_fixed_point(k++));
        }
        if(type == DataType::DOUBLE) {
          msg->value_[i] = convert<double>(msg->value_[i], encode, filterConf->num_bytes(), filterConf->mutable_fixed_point(k++));
        }
      }
    }
  };
