...
class StochasticDCA : public OptMethodBase<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType> {
...
public:
	...
	StochasticDCA(typename OptMethodBaseType::ModelSpecType& model) : OptMethodBaseType(model), sdcadesc_("Options for Sgd") {
		InitCmdOptions();
	}
	...
	void InitCmdOptions() {
		this->sdcadesc_.add_options()
			(this->kDualityGapOption,
				boost::program_options::value<double>(&this->dualgap_)->default_value(1e-3),
...
template <class ParameterType, class SampleType, class LabelType, class SparseGradientType, class DenseGradientType>
const char* StochasticDCA<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::kDualityGapOption = "sdca.gap";
