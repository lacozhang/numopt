...
template<class ParameterType,
	...
	class ProxGradientDescent : public OptMethodBase<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType> {
	public:
		...
		ProxGradientDescent(typename OptMethodBaseType::ModelSpecType& model)
			: OptMethodBaseType(model), proxgdesc_("Options for Proximal Gradient Descent") {
			InitCmdDescription();
			ResetState();
		}
		~ProxGradientDescent() {
		...
		virtual void InitFromCmd(int argc, const char* argv[]) override {
			...
			if (this->fixedstepsize_) {
				BOOST_LOG_TRIVIAL(info) << "Using fixed stepsize for proximal gradient descent";
			...
			if (this->learn_.l1_ == 0) {
				BOOST_LOG_TRIVIAL(warning) << "Do not use L1 regularization for Proximal gradient descent is like GD";
			}
		}
		...
		void InitCmdDescription() {
			this->proxgdesc_.add_options()
				(this->kConstStepSizeOption, 
					boost::program_options::value<bool>(&this->fixedstepsize_)->default_value(false), 
					"whether used fixed step size for proximal gradient descent")
				(this->kStepSizeOption, 
					boost::program_options::value<double>(&this->stepsize_)->default_value(0), 
...
const char* ProxGradientDescent<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::kConstStepSizeOption = "pgd.const";

template<class ParameterType, class SampleType, class LabelType, class SparseGradientType, class DenseGradientType>
const char* ProxGradientDescent<ParameterType, SampleType, LabelType, SparseGradientType, DenseGradientType>::kStepSizeOption = "pgd.stepsize";
